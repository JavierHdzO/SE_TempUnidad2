...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def createFiles(self):
        ...
        archivo = open("breast-cancer-wisconsin.csv", "r")  # ABRE EL ARCHIVO
        contenido = archivo.readlines()  # LEE TODO EL CONTENIDO DE ARCHIVO
        ...
        instanica = []
        for data in lista:
            aux = list(map(float, data[:9]))
            aux.append(int(data[9].replace("\n","")))
        ...
        fileEntrena = open("cancer_entrenamiento8713.csv","w")
        filePrueba = open("cancer_prueba8713.csv","w")
