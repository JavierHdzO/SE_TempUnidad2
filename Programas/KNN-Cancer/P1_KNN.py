...
archivo = open("EntrenamientoPrueba/cancer_entrenamiento8020.csv","r")
...
instancia = [ [ list(map(float,x[:9])), float(x[9].replace("\n","")) ] for x in lista ] #iris
...
prueba = [ [ list(map(float,x[:9])), float(x[9].replace("\n","")) ] for x in lista ] #iris
...
p = round(confusionMatrix[0][0] / ( confusionMatrix[0][0] + confusionMatrix[0][1]), 2)
print(f"\tP = {p}")

r = round(confusionMatrix[0][0] / ( confusionMatrix[0][0] + confusionMatrix[1][0] ), 2)
print(f"\tr = {r}")
